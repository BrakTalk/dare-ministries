...
- **Content data:** JSON files in `src/_data/` (site.json, projects.json) — edited directly in the repo
- **Database:** Netlify DB (managed Postgres). Schema migrations live in `netlify/database/migrations/` and are applied automatically on deploy. Tables: `volunteers`, `contacts`, `impact_stats`, `field_notes`, `field_note_photos`, `field_photo_submissions`, `field_photo_submission_files`, `field_photo_submission_events`, `user_profiles`, `portal_audit_log`.
...
- **Field Photo Inbox:** Resend sends signed `email.received` webhooks to `/api/inbound/field-photos`. Supported attachments are decoded and re-encoded without metadata, while an allowlisted EXIF subset is stored in `field_photo_submission_files`. Sanitized pending images remain in the private `field-photo-inbox` Blob store and are accessible only through `/api/admin/field-photo-inbox` to active coordinators. Approval copies the clean image into `field-photos`, stores the reviewed capture date/location label on `field_note_photos`, and scrubs exact GPS. A daily scheduled cleanup removes abandoned private photos. See `PHOTO_INBOX_SETUP.md` and `.env.example`.
...
This project is indexed by GitNexus as **dare-ministries** (2883 symbols, 6865 relationships, 58 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
